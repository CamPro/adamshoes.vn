...
if (!empty($_REQUEST['slug']))
 {
        $url=trim($_REQUEST['slug']);
        $brand_id=$db->getOne("select brand_id from ". $ecs->table("brand")." where url_seo='$url' " );
        $brand_id=$brand_id ? $brand_id : '';
 }
if (empty($brand_id))
{
    /* 缓存编号 */
    $cache_id = sprintf('%X', crc32($_CFG['lang'].'-'.$_COOKIE['client']));
...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.goods_desc_short, g.is_hot, g.is_best, g.is_new, g.is_promote, g.is_os,  g.is_special, g.goods_number, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.seller_note, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
