...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if (!$isadd)
    {
        ...
        create_html_editor('topic_intro', $topic['intro'],'FCintro','Normal', '100%', '300');
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
   ...
   // var_dump($_POST['topic_data']);
    ...
    if ($is_insert)
    {
        $sql = "INSERT INTO " . $ecs->table('topic') . " (title,url_seo,start_time,end_time,data,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description, discount_method)" .
                "VALUES ('$_POST[topic_name]', '$url_seo', '$start_time','$end_time','$data','$_POST[topic_intro]','$_POST[topic_template_file]','$_POST[topic_css]', '$topic_img', '$title_pic', '$base_style', '$htmls','$keywords','$description', '$_POST[discount_method]')";
    }
    else
    {
        $sql = "UPDATE " . $ecs->table('topic') .
                "SET title='$_POST[topic_name]', url_seo='$url_seo', start_time='$start_time',end_time='$end_time',data='$data',intro='$_POST[topic_intro]',template='$_POST[topic_template_file]',css='$_POST[topic_css]', topic_img='$topic_img', title_pic='$title_pic', base_style='$base_style', htmls='$htmls', keywords='$keywords', description='$description', discount_method = '$_POST[discount_method]'" .
               " WHERE topic_id='$topic_id' LIMIT 1";
        // xoa hinh cu
        if(!empty($_POST['title_img_url']) && empty($_FILES['title_pic'])){
            @unlink(ROOT_PATH.$_POST['title_img_url']);
        }
    }
