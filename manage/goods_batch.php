...
if ($_REQUEST['act'] == 'add')
{
    ...
    $lang_list = array('UTF8'      => $_LANG['charset']['utf8']);
    $download_list = array();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    else
    {
        ...
        $goods_sns = str_replace(', ', ',', $goods_sns);
    ...
    $sql = "SELECT DISTINCT goods_id, goods_sn, goods_name, shop_price, online_price, tragop_price, is_promote, promote_price, promote_start_date, promote_end_date, goods_gift, warranty, seller_note, timegift, is_real FROM " . $ecs->table('goods') . $where;
    $res = $db->getAll($sql);
    $goods = array();
   foreach ($res as $row) {
      ...
      $goods[$row['goods_id']]['online_price']              = $row['online_price'];
      $goods[$row['goods_id']]['tragop_price']              = $row['tragop_price'];
      $goods[$row['goods_id']]['is_promote']              = $row['is_promote'];
      $goods[$row['goods_id']]['promote_price']           = $row['promote_price'];
      $goods[$row['goods_id']]['promote_start_date']      = local_date('Y-m-d', $row['promote_start_date']);
      $goods[$row['goods_id']]['promote_end_date']        = local_date('Y-m-d', $row['promote_end_date']);
      $goods[$row['goods_id']]['goods_gift']              = $row['goods_gift'];
      $goods[$row['goods_id']]['warranty']                = $row['warranty'];
      $goods[$row['goods_id']]['seller_note']              = $row['seller_note'];
      $goods[$row['goods_id']]['timegift']                = $row['timegift'];
      $goods[$row['goods_id']]['is_real']                 = $row['is_real'];
   }
    ...
    if ($_POST['edit_method'] == 'each')
    {
        ...
        if (!empty($_POST['goods_id']))
        {
            foreach ($_POST['goods_id'] AS $goods_id)
            {
                ...
                $is_promote = !empty($_POST['is_promote'][$goods_id]) ? 1 : 0;
                $online_price = !empty($_POST['online_price'][$goods_id]) ? floatval($_POST['online_price'][$goods_id] ) : 0;
                $tragop_price = !empty($_POST['tragop_price'][$goods_id]) ? floatval($_POST['tragop_price'][$goods_id] ) : 0;
                $promote_price = ($is_promote == 1 && !empty($_POST['promote_price'][$goods_id])) ? floatval($_POST['promote_price'][$goods_id] ) : 0;
                $promote_start_date = ($is_promote == 1 && !empty($_POST['promote_start_date'][$goods_id])) ? local_strtotime($_POST['promote_start_date'][$goods_id]) : 0;
                $promote_end_date = ($is_promote == 1 && !empty($_POST['promote_end_date'][$goods_id])) ? local_strtotime($_POST['promote_end_date'][$goods_id]) : 0;
                $goods_gift = !empty($_POST['goods_gift'][$goods_id]) ? trim($_POST['goods_gift'][$goods_id]) : '';
                $seller_note = !empty($_POST['seller_note'][$goods_id]) ? trim($_POST['seller_note'][$goods_id]) : '';
                $timegift = !empty($_POST['timegift'][$goods_id]) ? trim($_POST['timegift'][$goods_id]) : '';
                $warranty = !empty($_POST['warranty'][$goods_id]) ? trim($_POST['warranty'][$goods_id]) : '';
                $goods = array(
                    ...
                    'goods_gift'      => trim($goods_gift),
                    'seller_note'      => trim($seller_note),
                    'timegift'      => trim($timegift),
