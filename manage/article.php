...
$allow_file_types = '|GIF|JPG|JPEG|PNG|BMP|SWF|DOC|DOCX|XLS|XLSX|PPT|PPTX|MID|WAV|ZIP|RAR|PDF|CHM|RM|TXT|';
...

{
    ...
    create_html_editor('content', '', 'FCKcontent', 'Normal', '100%', '500');

    create_html_editor('short_content', '', 'FCKshort_content', 'Basic', '50%', '200');
...

{
    ...
    //nobj url sao phai dat truoc thumnail de lay ten cho dep

    $url_seo = trim($_POST['url_seo']);

    if(!$exc->is_only('url_seo', $_POST['url_seo'], 0, " cat_id ='$_POST[article_cat]'"))

    {

         $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');

         sys_msg($_LANG['url_seo_exits'], 0, $link);
...

    {
        ...
        if (!check_type_allow($_FILES['article_thumb']['name'], $allow_image_types))
       ...
       // $res2 = upload_article_file($_FILES['article_thumb']);

        $image = new cls_image($_CFG['bgcolor']);

        $filename = $url_seo.'-'.$_CFG['article_thumb_width'].'x'.$_CFG['article_thumb_height'];

        $filename_small = $url_seo.'-'.$_CFG['article_small_thumb_width'].'x'.$_CFG['article_small_thumb_height'];

        $thumbnail = $image->make_thumb($_FILES['article_thumb']['tmp_name'], $_CFG['article_thumb_width'],$_CFG['article_thumb_height'], ROOT_PATH .'data/article/thumb/', '#ffffff', $filename);

        $small_thumb = $image->make_thumb($_FILES['article_thumb']['tmp_name'], $_CFG['article_small_thumb_width'],$_CFG['article_small_thumb_height'], ROOT_PATH .'data/article/thumb/', '#ffffff', $filename_small);
    ...
    $off_time = !empty($_POST['off_time']) ? local_strtotime($_POST['off_time']) : 0;
    ...
    $content = $flagurl == 1 ? removeTag('a', $content) : $content;

     //$string, $prefix, $flag

    if($flagimg == 1){

        $content  = autoGetImages($content, 'article', $flagimg);

    }



    $sql = "INSERT INTO ".$ecs->table('article')."(title, url_seo, article_thumb, article_small_thumb, cat_id, article_type, is_open, author, ".

                "author_email,meta_title, keywords, content, add_time, off_time, file_url, open_type, link, description) ".

            "VALUES ('$_POST[title]', '$url_seo', '$thumbnail', '$small_thumb', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".

                "'$_POST[author]', '$_POST[author_email]', '$_POST[meta_title]', '$_POST[keywords]', '$content', ".

                "'$add_time', '$off_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]')";
...

{
    ...
    $article['off_time'] = local_date('Y-m-d', $article['off_time']);



   /* 创建 html editor */

   create_html_editor('content', $article['content'], 'FCKcontent', 'Normal', '100%', '500');
    ...
    $article_rela = get_articles_rela($_REQUEST['id']);
...

{
    ...
    if($url_seo != $_POST['old_url_seo']){

        if(!$exc->is_only('url_seo', $_POST['url_seo'], $_POST['id']))
    ...
    $sql = "SELECT article_thumb, article_small_thumb FROM " . $ecs->table('article') . " WHERE article_id = '$_POST[id]'";

    $old_url = $db->getAll($sql);

    if ($small_thumb == '')

    {

        $small_thumb = $old_url[0]['article_small_thumb'];

    }



    if ($old_url[0]['article_thumb'] != '' && $old_url[0]['article_thumb'] != $thumbnail && strpos($old_url[0]['article_thumb'], 'http://') === false && strpos($old_url[0]['article_thumb'], 'https://') === false)

    {

        @unlink(ROOT_PATH . $old_url[0]['article_thumb']);

    }

    if ($old_url[0]['article_small_thumb'] != '' && $old_url[0]['article_small_thumb'] != $small_thumb && strpos($old_url[0]['article_small_thumb'], 'http://') === false && strpos($old_url[0]['article_small_thumb'], 'https://') === false)

    {

        @unlink(ROOT_PATH . $old_url[0]['article_small_thumb']);
    ...
    if ($exc->edit("title='$_POST[title]', url_seo ='$url_seo', article_thumb ='$thumbnail', article_small_thumb ='$small_thumb', cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', author='$_POST[author]', author_email='$_POST[author_email]', meta_title ='$_POST[meta_title]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', content='$content', link='$_POST[link_url]', update_time='$update_time', off_time='$off_time', description = '$_POST[description]'", $_POST['id']))
...
elseif ($_REQUEST['act'] == 'add_article_rela')

{
...

    {
        ...
        $sql = "INSERT INTO " . $ecs->table('article_rela') . " (article_id, article_id_rela, admin_id) " .

                "VALUES ('$article_id', '$val', '$_SESSION[admin_id]')";
    ...
    $arr = get_articles_rela($article_id);
...

    {
        ...
        $opt[] = array('value'      => $val['article_id_rela'],
...
elseif ($_REQUEST['act'] == 'drop_article_rela')

{
    ...
    $sql = "DELETE FROM " .$ecs->table('article_rela') . " WHERE " . db_create_in($articles, "article_id_rela") . " AND article_id = '$article_id'";
 ...
 * Get danh sach tin tuc lien quan
...
function get_articles_rela($article_id)

{

    $sql = "SELECT ar.article_id_rela, a.title " .
            ...
            "AND ar.article_id_rela = a.article_id ";

    if ($article_id == 0)

    {

        $sql .= " AND ar.admin_id = '$_SESSION[admin_id]'";
