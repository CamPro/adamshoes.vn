...

{
    ...
    $smarty->assign('intro_list',   get_intro_list()+array('no_content'=>"No Content"));
...

{
    ...
    $sql = "SELECT giasoc_id, giasoc_name FROM " . $ecs->table('giasoc');

    $giasoc = $db->getAll($sql);



    $smarty->assign('giasoc', $giasoc);
    ...
    //update link and save remote link img

   // $goods['goods_desc']  = autoGetImages($goods['goods_desc'], 'goods');
   ...
   // $goods_name_style = explode('+', empty($goods['goods_name_style']) ? '+' : $goods['goods_name_style']);
    ...
    create_html_editor('goods_desc', $goods['goods_desc'], 'FCKeditor', 'Normal', '100%', '400');

    create_html_editor('img_special',$goods['img_special'], 'img_special', 'Image', '50%', '200');
...

{
...

    {
...

        {
            ...
            $sql = "SELECT goods_thumb, goods_thumb_small, goods_img, original_img " .
            ...
            if ($row['goods_thumb_small'] != '' && is_file('../' . $row['goods_thumb_small']))

            {

                @unlink('../' . $row['goods_thumb_small']);
...

        {
            ...
            $fname =  $image->get_namefile($_FILES['goods_img']['name']);

            $original_img   = $image->upload_image($_FILES['goods_img'],'',$fname); // goods normal name
        ...
        //     $img        = $original_img;   // Gallery Photo

        //     $pos        = strpos(basename($img), '.');

        //     $filename   = array_shift(explode(".", basename($img)));

        //     $newname    = dirname($img) . '/' . $filename . substr(basename($img), $pos);

        //     if (!copy('../' . $img, '../' . $newname))

        //     {

        //         sys_msg('fail to copy file 1: ' . realpath('../' . $img), 1, array(), false);
        ...
        //     $image->load(ROOT_PATH.$img);

        //     $image->resize($GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height']);

        //     $image->save(ROOT_PATH.$img);
        ...
        // Resize hình ảnh SP ở trang chi tiết từ hình vừa lên
...

        {
...

            {
...

                {
                    ...
                    $abc = explode(".", basename($goods_img));

                    $gname  = array_shift($abc);

                    $pathg = ROOT_PATH . IMAGE_DIR . '/' . date('Ym').'/goods_img/';

                    $goods_img = $image->make_thumb('../'. $goods_img , $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height'],$pathg, '', 'g-'.$gname);
                ...
                //         sys_msg('fail to copy file 2: ' . realpath('../' . $img), 1, array(), false);
...

    {
        ...
        $tname =  $image->get_namefile($_FILES['goods_thumb']['name']);

        $goods_thumb = $image->upload_image($_FILES['goods_thumb'], '',  'thumb-'.$tname); //name
        ...
        // tao thumb nho de hien thi cột dọc 120x120px

        if ($_CFG['small_thumb_width'] != 0 || $_CFG['small_thumb_height'] != 0)

        {

            $stname  = array_shift(explode(".", basename($goods_thumb)));

            $paththumb = ROOT_PATH . IMAGE_DIR . '/' . date('Ym').'/thumb_img/';

            $goods_thumb_small = $image->make_thumb('../' . $goods_thumb, $GLOBALS['_CFG']['small_thumb_width'],  $GLOBALS['_CFG']['small_thumb_height'], $paththumb, '', 's'.$stname);

            if ($goods_thumb_small === false)
...

    {
...

        {
...

            {
                ...
                $def = explode(".", basename($original_img));

                $tname  = array_shift($def);

                $goods_thumb = $image->make_thumb('../' . $original_img, $GLOBALS['_CFG']['thumb_width'],  $GLOBALS['_CFG']['thumb_height'],  $paththumb, '', 'thumb-'.$tname );
...

            {
             ...
             // make small thumb goods

            if ($_CFG['small_thumb_width'] != 0 || $_CFG['small_thumb_height'] != 0)

            {

                $ghi = explode(".", basename($original_img));

                $stname  = array_shift($ghi);

                $goods_thumb_small = $image->make_thumb('../' . $original_img, $GLOBALS['_CFG']['small_thumb_width'],  $GLOBALS['_CFG']['small_thumb_height'], $paththumb, '', 'sthumb-'.$stname);
    ...
    $online_price = !empty($_POST['online_price']) ? $_POST['online_price'] : 0;

    $tragop_price = !empty($_POST['tragop_price']) ? $_POST['tragop_price'] : 0;
    ...
    $is_shock = isset($_POST['is_shock']) ? 1 : 0;
    
    if($_POST['goods_number'])  $label_status=$_POST['status_label'];else $label_status=$_POST['label_status'];
    ...
    $is_os= isset($_POST['is_os']) ? intval($_POST['is_os']) : 0;

    $is_tragop= isset($_POST['is_tragop']) ? intval($_POST['is_tragop']) : 0;

    $giasoc_id= isset($_POST['giasoc_id']) ? intval($_POST['giasoc_id']) : 0;

    $timegift= isset($_POST['timegift']) ? intval($_POST['timegift']) : 0;

    $is_preoder= isset($_POST['is_preoder']) ? intval($_POST['is_preoder']) : 0;
    ...
    $goods_thumb_small = (empty($goods_thumb_small) && isset($_POST['auto_thumb']))? $goods_img : $goods_thumb_small;

    $goods_name = empty($_POST['goods_name']) ? '' : $_POST['goods_name'];

    $goods_name =  filter_var($goods_name, FILTER_SANITIZE_STRING);

    $goods_brief = empty($_POST['goods_brief']) ? '' : $_POST['goods_brief'];

    $seller_note = empty($_POST['seller_note']) ? '' : $_POST['seller_note'];

    $img_special = empty($_POST['img_special']) ? '' : $_POST['img_special'];

    $warranty =     empty($_POST['warranty']) ? '' : $_POST['warranty'];
    
    $noibaohanh =     empty($_POST['noibaohanh']) ? 'bks' : $_POST['noibaohanh'];

    $show_warranty = isset($_POST['show_warranty']) ? intval($_POST['show_warranty']) : 1;

    $goods_title =     empty($_POST['goods_title']) ? '' : $_POST['goods_title'];

    $url_seo =     empty($_POST['url_seo']) ? '' : $_POST['url_seo'];

    $description =     empty($_POST['description']) ? '' : $_POST['description'];

    $goods_desc_short =     empty($_POST['goods_desc_short']) ? '' : $_POST['goods_desc_short'];

    $goods_desc_long =     empty($_POST['goods_desc_long']) ? '' : $_POST['goods_desc_long'];

    $pack =     empty($_POST['pack']) ? '' : $_POST['pack'];

    $goods_gift =     empty($_POST['goods_gift']) ? '' : $_POST['goods_gift'];

    $online_gift =     empty($_POST['online_gift']) ? '' : $_POST['online_gift'];

    $tragop_gift =     empty($_POST['tragop_gift']) ? '' : $_POST['tragop_gift'];

    $thutuctg =     empty($_POST['thutuctg']) ? '' : $_POST['thutuctg'];
   ...
   //xu li khi copy noi dung tu site khac

    $flagimg = isset($_POST['flagimage']) ? 1 : 0;

    $flagurl = isset($_POST['flagurl']) ? 1 : 0;

    $goods_desc = $flagurl == 1 ? removeTag('a', $goods_desc) : $goods_desc;
...

    {
...

        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (`goods_name`, `goods_sn`, " .
                    ...
                    "`cat_id`, `brand_id`, `shop_price`, `market_price`, `online_price`, `tragop_price`, `is_promote`, `promote_price`, " .

                    "`promote_start_date`, `promote_end_date`, `goods_img`, `goods_thumb`, `goods_thumb_small`, `original_img`, `keywords`, `goods_brief`, " .

                    "`seller_note`, `img_special`, `warranty`, `show_warranty`,`goods_title`, `url_seo`, `description`, `goods_desc_short`, `goods_desc_long`, `pack`, `goods_gift`, `online_gift`, `tragop_gift`, `thutuctg`, `goods_weight`, `goods_number`, `warn_number`, `integral`, `give_integral`, `is_best`, `is_new`, `is_hot`, `is_shock`, " .

                    "`is_on_sale`, `is_alone_sale`, `is_shipping`, `is_os`, `is_tragop`, `giasoc_id`,`timegift`, `is_preoder`, `is_special`, `goods_desc`, `add_time`, `last_update`, `goods_type`, `rank_integral`, `suppliers_id`,`noibaohanh`,`label_status`)" .

                    " VALUES ('$goods_name', '$goods_sn', '$catgory_id', " .

                    "'$brand_id', '$shop_price', '$market_price', '$online_price', '$tragop_price','$is_promote','$promote_price', ".

                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$goods_thumb_small', '$original_img', ".

                    "'$keywords', '$goods_brief', '$seller_note', 'img_special','$warranty', '$show_warranty', '$goods_title', '$url_seo', '$description', '$goods_desc_short', '$goods_desc_long', '$pack','$goods_gift', '$online_gift', '$tragop_gift','$thutuctg', '$goods_weight', '$goods_number',".

                    "'$warn_number', '$integral', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_shock', '$is_on_sale', '$is_alone_sale', '$is_shipping', '$is_os', '$is_tragop', '$giasoc_id','$timegift', '$is_preoder', '$is_special', ".

                    "'$goods_desc', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$noibaohanh','$label_status')";
...

        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_sn, " .
                    ...
                    "`warranty`, `show_warranty`, `goods_title`, `url_seo`, `description`, `goods_desc_short`, `goods_desc_long`, `pack`, `goods_gift`, `online_gift`, `tragop_gift`, `thutuctg`, ".

                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .

                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_shock, is_real, " .

                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,noibaohanh,label_status)" .
                ...
                "VALUES ('$_POST[goods_name]', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_shock',  0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".

                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$noibaohanh','$label_status')";
...

    {
...

        {
            ...
            @unlink(ROOT_PATH . $row['goods_thumb_small']);

        }



        $goods_name =  filter_var($_POST['goods_name'], FILTER_SANITIZE_STRING);

        $url_seo =  filter_var($_POST['url_seo'], FILTER_SANITIZE_STRING);
        ...
        if ($goods_thumb_small)

        {

            $sql .= "goods_thumb_small = '$goods_thumb_small', ";
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "img_special = '$_POST[img_special]', " .

                "seller_note = '$_POST[seller_note]', " .

                "warranty    = '$_POST[warranty]', " .
                "noibaohanh    = '$_POST[noibaohanh]', " .
                 "label_status    = '$label_status', " .
                "show_warranty    = '$_POST[show_warranty]', " .
                ...
                "goods_desc_short = '$goods_desc_short', " .

                "goods_desc_long = '$goods_desc_long', " .
...

    {
        ...
        $keywords = trim_right($_POST['keywords']);
    ...
    $goods_thumb_small = reformat_image_name('goods_thumb_small', $goods_id, $goods_thumb_small, 'thumb');
...

    {
        ...
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_thumb_small = '$goods_thumb_small' WHERE goods_id='$goods_id'");
...

{
...

    {
...

        {
            ...
            update_goods($goods_id, 'is_shock', '1');
...

        {
            ...
            update_goods($goods_id, 'is_shock', '0');
...

{
...

    {
        ...
        if ($exc->edit("promote_price = '$goods_price'", $goods_id))
...
elseif ($_REQUEST['act'] == 'edit_tragop_price')

{
...

    {
        ...
        if ($exc->edit("tragop_price = '$goods_price'", $goods_id))
...
elseif ($_REQUEST['act'] == 'toggle_shock')

{
    ...
    $is_shock         = intval($_POST['val']);



    if ($exc->edit("is_shock = '$is_shock', last_update=" .gmtime(), $goods_id))

    {

        clear_cache_files();

        make_json_result($is_shock);
...

{
    ...
    $sql = "SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, goods_thumb_small, " .
    ...
    if (!empty($goods['goods_thumb_small']))

    {

        @unlink('../' . $goods['goods_thumb_small']);
