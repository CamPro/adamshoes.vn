...
if ($_REQUEST['act'] == 'login')
{
    ...
    $page = $_mobile == 1 ? 'login_mobile.htm' : 'login.htm' ;
    $smarty->display($page);
}
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, fullname, email, password, add_time, nav_list, action_list, role_id) ".
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['fullname'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id')";
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, fullname, email, password, agency_id, role_id FROM " .$ecs->table('admin_user').
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    $admin_fullname  = !empty($_REQUEST['fullname']) ? trim($_REQUEST['fullname']) : '';
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 获得当前管理员数据信息 */
    $sql = "SELECT user_id, user_name, fullname, email, nav_list ".
...
{
    $list = array();
    $sql  = 'SELECT user_id, user_name, fullname, email, add_time, last_login '.
