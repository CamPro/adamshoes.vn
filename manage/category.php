...

{
    ...
    $cat['cat_url_seo']  = !empty($_POST['cat_url_seo'])  ? trim($_POST['cat_url_seo'])     : '';
    ...
    if (!$exc->is_only('cat_url_seo', $cat['cat_url_seo'], $cat_id))

    {

         sys_msg($_LANG['url_seo_exists'], 1, array(), false);
...

{
    ...
    if (empty($cat['cat_url_seo']))

    {

         sys_msg($_LANG['cat_url_seo_empty'], 1, array(), false);

    }



      if ($cat['cat_url_seo'] != $old_url_seo)

    {

        if (cat_exists($cat['cat_url_seo'], $cat['parent_id']))

        {
           ...
           sys_msg($_LANG['url_seo_exists'], 0, $link);
