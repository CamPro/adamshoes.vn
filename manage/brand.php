...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $is_only = $exc->is_only('url_seo', $_POST['url_seo']);

    if (!$is_only)
    {
        sys_msg(sprintf('Url seo đã tồn tại', stripslashes($_POST['url_seo'])), 1);
    }
     if (empty($_POST['url_seo']))
    {
         sys_msg('URl seo không thể để trống', 1, array(), false);
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, url_seo, site_url, brand_desc, brand_logo, is_show, sort_order) ".
           "VALUES ('$_POST[brand_name]', '$_POST[url_seo]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$_POST[sort_order]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, url_seo, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order ".
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    if ($_POST['url_seo'] != $_POST['old_url_seo'])
    {
        $is_only = $exc->is_only('url_seo', $_POST['url_seo']);
    ...
    /* 处理图片 */
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $param = "brand_name = '$_POST[brand_name]',  url_seo = '$_POST[url_seo]', site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]' ";
    if (!empty($img_name))
    {
        //有图片上传
...
elseif($_REQUEST['act'] == 'add_brand')
{
    ...
    {
        $url_seo = seo_url($brand);
        $sql = "INSERT INTO " . $ecs->table('brand') . "(brand_name,url_seo)" .
               "VALUES ( '$brand', '$url_seo')";
