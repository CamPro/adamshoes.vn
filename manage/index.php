...
if ($_REQUEST['act'] == '')
{
    ...
    $smarty->assign('shop_url', urlencode($ecs->url()));
     $page = $_mobile == 1 ? 'index_mobile.htm' : 'index.htm';
    $smarty->display($page);
}
...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    $page = $_mobile == 1 ? 'start_mobile.htm' : 'start.htm';
    $smarty->display($page);
}
...
elseif ($_REQUEST['act'] == 'check_order')
{
    if (empty($_SESSION['last_check']))
    {
        ...
        make_json_result('', '', array('new_order' => 0, 'new_comment' => 0));
    }

    /* 新订单 */
    $sql = 'SELECT COUNT(*) FROM ' . $ecs->table('order').
    " WHERE add_time >= '$_SESSION[last_check]'";
    $arr['new_order'] = $db->getOne($sql);

    /* count comment */
    $sql = 'SELECT COUNT(*) FROM '.$ecs->table('comment').
    ' WHERE add_time >= ' . $_SESSION['last_check'];
    $arr['new_comment'] = $db->getOne($sql);


    $_SESSION['last_check'] = gmtime();

    if (!(is_numeric($arr['new_order']) && is_numeric($arr['new_comment'])))
    {
