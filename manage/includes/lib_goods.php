...

{
...

    {
        ...
        $name  = $GLOBALS['image']->get_namefile($image_files['name'][$key]);
...

        {
            ...
            //     $thumb_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key], $GLOBALS['_CFG']['small_thumb_width'],  $GLOBALS['_CFG']['small_thumb_height'], '', '', $name.'-thumb');
            ...
            $img_original = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key], $GLOBALS['_CFG']['gallery_width'],  $GLOBALS['_CFG']['gallery_height'], '', '', $name);
...

            {
                ...
                $dsd = explode(".", basename($img_original));

                $filename   = array_shift($dsd);

                $newname    = dirname($img_original) . '/' . $filename.'-g' . substr(basename($img_original), $pos);
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc,  img_original) " .

                    "VALUES ('$goods_id', '$img_url', '$img_desc',  '$img_original')";
...

        {
            ...
            //     $thumb_url = $GLOBALS['image']->make_thumb($down_img, $GLOBALS['_CFG']['small_thumb_width'],  $GLOBALS['_CFG']['small_thumb_height']);
...

{
...

    {
...

        {
           ...
           case 'no_content': 
                $where.=" AND ( goods_desc  is NULL OR  goods_desc=''   ) ";break;     
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_gift, goods_sn, shop_price, promote_price, tragop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
...

{
...

    {
        ...
        case 'goods':
            ...
            $img_name = $goods_id . '-' . $rand_name;

            break;

        case 'goods_thumb':

            $img_name = $goods_id . '-g' . $rand_name;

            break;

         case 'goods_thumb_small':

             $img_name = $goods_id . '-ps' . $rand_name;

             break;

        case 'gallery':

            $img_name = $goods_id . '-p' . $rand_name;

            break;

        case 'gallery_thumb':

            $img_name = $goods_id . '-pt' . $rand_name;
