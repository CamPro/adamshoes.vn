...
function sys_msg($msg_detail, $msg_type = 0, $links = array(), $auto_redirect = true)
{
    ...
    $_mobile = isset($_COOKIE['client']) && $_COOKIE['client'] == 'mobile' ? 1 : 0;
    $pmsg = $_mobile ? 'message_mobile.htm' : 'message.htm';
    $GLOBALS['smarty']->display($pmsg);
...
/**
 * edit buy nobj nguyen
 * @param   string  input_name
 * @param   string  input_value
 * @param   string  output_value
  * @param   string  ToolbarSet with 4 value   Default | Basic | Normal | Mail
 */
function create_html_editor($input_name, $input_value = '', $output_name, $ToolbarSet, $Width, $Height)
{
    global $smarty;

    $editor = new FCKeditor($input_name);
    $editor->BasePath   = '../includes/fckeditor/';
    $editor->ToolbarSet = $ToolbarSet; // Default | Basic | Normal | Mail
    $editor->Width      = $Width; // ex 100%
    $editor->Height     = $Height; //ex 320
    $editor->Value      = $input_value;
    $input_name         = $editor->CreateHtml();
    $smarty->assign($output_name, $input_name);
}
...
function get_goods_list($filter)
{
    ...
    /* 取得数据 */
    $sql = 'SELECT goods_id, goods_sn, is_promote, goods_name, shop_price '.
...
/*
* @Param $tag       string      exp a -> <a>
* @Param $string    string      code html
*/
...
/*
* Dieu kien de chay ham la phat hien co link anh tu ben ngoai
* @Param $string    string      code html
* @Param $path      string      directory to save images
* @Param $prefix    string      prefix name combine old name file
*/

function callbackFunction_article($matches){
        $filename = basename($matches[1]); //tach lay ten file ==> hinh-anh-gi-do.jpg
        $savename = 'data/article/photo/bkit-'.$filename;
        file_put_contents(ROOT_PATH.$savename,file_get_contents($matches[1])); //luu file
        return 'src='.$savename; // update link moi
}
function callbackFunction_goods($matches){
        $filename = basename($matches[1]); //tach lay ten file ==> hinh-anh-gi-do.jpg
        $savename = 'data/goods_article/bkit-'.$filename;
       // $img = file_get_contents($matches[1]);
        //file_put_contents(ROOT_PATH.$savename,$img); //luu file
        return 'src='.$savename; // update link moi
}
function autoGetImages($string, $prefix, $flag){
    if($flag == 1){
        $string = preg_replace('/data-original="([^"]*)"/i', '', $string);  //xoa link data-original neu co
        $string = preg_replace('/class="([^"]*)"/i', '', $string);  // xoa class css neu co
        $string = preg_replace_callback('/src=\\\"([^\"]*)\\\"/i', 'callbackFunction_'.$prefix, $string);
        return $string;
    }
    return $string;
}
