...
elseif ($_REQUEST['act'] =='insert')
{
    ...
    // thumnail nobj
     $thumbnail = '';
    if (isset($_FILES['package_img']) && $_FILES['package_img']['tmp_name'] != '' &&
        isset($_FILES['package_img']['tmp_name']) &&$_FILES['package_img']['tmp_name'] != 'none')
    {   
        if (!check_type_allow($_FILES['package_img']['name'], $allow_image_types))
        {
       ...
       // $res2 = upload_article_file($_FILES['article_thumb']);
        $image = new cls_image();
        $filename = $url_seo.'-'.$_CFG['image_width'].'x'.$_CFG['image_height'];
        $thumbnail = $image->make_thumb($_FILES['package_img']['tmp_name'], $_CFG['image_height'],$_CFG['image_height'], ROOT_PATH .'data/packageimg/', '#ffffff', $filename);
        
        if ($thumbnail === false)
        {
            sys_msg($image->error_msg('Have a Error !'), 1, array(), false);
        }
    }
    ...
    /* 插入数据 */
    $record = array('act_name'=>$_POST['package_name'], 'url_seo'=>$_POST['url_seo'], 'photo'=>$thumbnail,'act_desc'=>$_POST['desc'],
...
elseif ($_REQUEST['act'] =='update')
{
    ...
    if ($thumbnail == '')
    {
        $thumbnail = $_POST['old_package_img'];
    }
    ...
    /* 更新数据 */
    $record = array('act_name' => $_POST['package_name'], 'url_seo'=>$url_seo, 'photo'=>$thumbnail, 'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'],
...
elseif($_REQUEST['act'] == 'drop_thumb')
{
    ...
    /* 取得logo名称 */
    $sql = "SELECT photo FROM " .$ecs->table('goods_activity'). " WHERE act_id = '$act_id'";
    $img_name = $db->getOne($sql);

    if (!empty($img_name))
    {
        @unlink(ROOT_PATH.$img_name);
        $sql = "UPDATE " .$ecs->table('goods_activity'). " SET photo = '' WHERE act_id = '$act_id'";
        $db->query($sql);
    }
    
    clear_cache_files();
     $link= array(array("text" => "Packages list", "href"=>"package.php?act=edit&id=$act_id"));
     sys_msg('Delete Success !', 0, $link);
}
