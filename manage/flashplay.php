...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_POST['step'] == 2)
    {
        if (!empty($_FILES['img_file_src']['name']))
        {
            ...
            $image = new cls_image();
            $name = $image->get_namefile($_FILES['img_file_src']['name']);
            $name .= date('Ymd');
            // for ($i = 0; $i < 6; $i++)
            // {
            //     $name .= chr(mt_rand(97, 122));
            // }
            $dfv = explode('.', $_FILES['img_file_src']['name']);
            $name .= '.' . end($dfv);
...
elseif ($_REQUEST['act']== 'custom_list')
{
    /* 标签初始化 */
    $group_list = array(
        'sys' => array('text' => $_LANG['system_set'], 'url' => 'flashplay.php?act=list'),
...
//        }
    ...
    /* 添加 */
    $ad = array('ad_name' => '', 'ad_type' => 0, 'ad_url' => '', 'htmls' => '',
                'ad_status' =>'1', 'sort' =>'9', 'ad_id' => '0', 'url' => '#');
    $smarty->assign('ad', $ad);
    $smarty->assign('form_act', 'custom_insert');
...
elseif ($_REQUEST['act'] == 'custom_insert')
{
    ...
    if ($ad_img['ad_img']['name'] && $ad_img['ad_img']['size'] > 0)
    {
        ...
        // }
        $image = new cls_image();
        $name = $image->get_namefile($_FILES['ad_img']['name']);
        $name .= '.' . explode('.', $ad_img['ad_img']['name']);
        $name = end($name);
        $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;
    ...
    switch ($filter['ad']['ad_type'])
    {
     ...
     $filter['ad']['sort'] = !empty($filter['ad']['sort']) ? intval($filter['ad']['sort']) : 9;
...
elseif ($_REQUEST['act'] == 'custom_edit')
{
    ...
    /* 查询自定义广告信息 */
    $sql = "SELECT ad_id, ad_type, content, url, ad_status, ad_name, sort FROM " . $GLOBALS['ecs']->table("ad_custom") . " WHERE ad_id = $id LIMIT 0, 1";
...
elseif ($_REQUEST['act'] == 'custom_update')
{
    ...
    if ($ad_img['ad_img']['name'] && $ad_img['ad_img']['size'] > 0)
    {
        ...
        $image = new cls_image();
        $name = $image->get_namefile($_FILES['ad_img']['name']);
        $dad = explode('.', $ad_img['ad_img']['name']);
        $name .= '.' . end($dad);
        $target = ROOT_PATH . DATA_DIR . '/afficheimg/' . $name;
...
function ad_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 查询 */
        $sql = "SELECT ad_id, CASE WHEN ad_type = 0 THEN 'Ảnh'
                                   WHEN ad_type = 1 THEN 'Flash'
                                   WHEN ad_type = 2 THEN 'Code'
                                   WHEN ad_type = 3 THEN 'Character'
                                   ELSE '' END AS type_name, ad_name, add_time, sort, CASE WHEN ad_status = 1 THEN '启用' ELSE '关闭' END AS status_name, ad_type, ad_status
