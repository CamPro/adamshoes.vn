...
function goituvan(id,goodsname, code, price, promo){
    ...
    if(error == 1){
       $.fn.colorbox({width: '97%',html:'<div class="message_box mb_info">'+$msg+'</div>'});
    }
    ...
    if(error == 0){
    	...
    	function(response){
    		var res = $.evalJSON(response);
    		$.fn.colorbox({width: '97%',html:'<div class="message_box mb_info">'+res.message+'</div>'});
            //reset form
			...
			if (res.error > 0) {
				...
				if (res.one_step_buy == '1') {
					...
					if ($('#page_flow').length > 0) {
						...
						$.fn.colorbox({width: '97%',html:'<div class="message_box mb_info">' + lang.add_to_cart_success + '<p class="action"><a href="flow.php?step=cart" class="button brighter_button"><span>' + lang.checkout_now + '</span></a><a href="javascript:void(0);" class="tool_link" onclick="$.fn.colorbox.close(); return false;">' + lang.continue_browsing_products + '</a></p></div>'});
					};
				}
			}
		},
...
function addPackageToCart(id) {
	...
	$.post(
		...
		function(response){
			...
			if (res.error > 0) {
				if (res.error == 2) {
					$.fn.colorbox({width: '97%',html:'<div class="message_box mb_question">' + res.message + '<p class="action"><a href="user.php?act=add_booking&id=' + res.goods_id + '" class="button brighter_button"><span>' + lang.booking + '</span></a><a href="javascript:void(0);" class="tool_link" onclick="$.fn.colorbox.close(); return false;">' + lang.cancel + '</a></p></div>'});
				}
				else {
					$.fn.colorbox({width: '97%',html:'<div class="message_box mb_info">' + res.message + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'});
				}
...
function submitComment(form) {
	...
	$.post(
		...
		function(response){
            ...
            /*
			if (res.message) {
				$.fn.colorbox({width: '97%',html:'<div class="message_box mb_info">' + res.message + '<p class="action"><a href="javascript:void(0);" class="button" onclick="$.fn.colorbox.close(); return false;"><span>' + lang.confirm + '</span></a></p></div>'});
			}
            */
			if (res.error == 0) {
...
function cLink(content, link) {
    $.fn.colorbox({width: '97%',transition:'none',html:'<div class="message_box mb_info">' + content + '<p class="action"><a href="'+link+'" class="button brighter_button"><span>OK</span></a></p></div>'});
}
