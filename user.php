...
if ($action == 'default')
{
    ...
    $display_mode = isset($_COOKIE['client']) ? 'user_clips_mobile.dwt' : 'user_clips.dwt';
...
if ($action == 'register')
{
    ...
    {
        $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'thanh-vien') ? './home' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    }
...
//    $smarty->assign('back_act', $back_act);
    $display_mode = isset($_COOKIE['client']) ? 'user_passport_mobile.dwt' : 'user_passport.dwt';
    $smarty->display($display_mode);
}
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if ((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
        {
            ...
            {
                show_message($_LANG['invalid_captcha'], $_LANG['sign_up'], 'dang-ky', 'error');
            }
        ...
        {
            $err->show($_LANG['sign_up'], 'dang-ky');
        }
    }
}
...
elseif ($action == 'act_login')
{
    ...
    if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
    {
        if (empty($_POST['captcha']))
        {
            show_message($_LANG['invalid_captcha'], $_LANG['relogin_lnk'], 'thanh-vien', 'error');
        }
    ...
    {
        $_SESSION['login_fail'] ++ ;
        show_message($_LANG['login_failure'], $_LANG['relogin_lnk'], 'thanh-vien', 'error');
    }
}
...
elseif ($action == 'profile')
{
    ...
    $display_mode = isset($_COOKIE['client']) ? 'user_transaction_mobile.dwt' : 'user_transaction.dwt';
    $smarty->display($display_mode);
}
