...
function group_buy_list($size, $page)
{
    ...
    $gb_list = array();
    $now = gmtime();
    $sql = "SELECT b.*, g.shop_price, g.click_count, g.goods_gift, IFNULL(g.goods_thumb, '') AS goods_thumb, b.act_id AS group_buy_id, ".
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $price    = $group_buy['shop_price'];
        $nowprice = $price_ladder[0]['price'];
        $group_buy['savemoney'] = $price-$nowprice;
        $group_buy['savemoney'] = price_format($group_buy['savemoney']);
        if($nowprice > 0)
        {
            $group_buy['salerate'] = round(10 / ($price / $nowprice), 1);
        }
        else
        {
            $group_buy['salerate'] = 0;
        }
        $group_buy['shop_price'] = price_format($group_buy['shop_price']);
        $group_buy['price_group_buy'] = price_format($price_ladder[0]['price']);
