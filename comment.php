...

{
    ...
    if(isset($cmt->telephone))
        $telephone = empty($cmt->telephone) ? $_SESSION['telephone'] : trim($cmt->telephone);    
    ...
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('comment') .
           ...
           "(comment_type, id_value, email,telephone, user_name, content, comment_rank, add_time, ip_address, status, parent_id, user_id) VALUES " .

           "('" .$cmt->type. "', '" .$cmt->id. "', '$email','$telephone', '$user_name', '" .$cmt->content."', '".$cmt->rank."', ".gmtime().", '".real_ip()."', '$status', '0', '$user_id')";
