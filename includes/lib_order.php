...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['formated_goods_price']  = price_format($value['goods_price']);
 ...
 * @param   integer $game    game: 0 = nothing | 1 : online | 2 : gia soc
 * @return  boolean
 */
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0,$game)
{
    $GLOBALS['err']->clean();
    $_parent_id = $parent;

    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_gift, g.online_gift, g.goods_sn, g.is_on_sale, g.is_real, ".
                "g.market_price, g.online_price, g.shop_price AS org_price, g.promote_price, g.promote_start_date, ".
    ...
    $goods_price            = get_final_price($goods_id, $num, true, $spec,$game=0);
    ...
    if($goods['online_price'] > 0){
        $goods_gift = $goods['online_gift'] != '' ? $goods['online_gift'] : '';
    }else{
        $goods_gift = $goods['goods_gift'];
    }

    /* lay thong tind e dua vao cart */
    $parent = array(
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            {
                $goods_price = get_final_price($goods_id, $num, true, $spec,$game=0);
                // tinh giam tru gia online
                /*if($game == 1){
                    $goods_price = $goods['online_price'] > 0 ? $goods_price - $goods['online_price'] : $goods_price;
                }*/
...
function get_cart_goods()
{
    ...
    $total = array(
        ...
        'shipping_fee'=>0 ,  // phi van chuyen cho don hang
        'shipping_fee_origion'=>0,
        'subtotal'    =>0  
    );
    ...
    /*eseo tuy chon phi ship*/
    $min_free_ship = 1000000;
    $ship_fee = 30000;
    $total['subtotal_origion']=($total['goods_price']);
    $total['subtotal']=price_format($total['goods_price']);
    if($total['goods_price']   < $min_free_ship )
    {
        $total['shipping_fee_origion'] = $ship_fee;
        $total['shipping_fee'] = price_format($ship_fee);
        $total['goods_price'] += $ship_fee;
        $total['market_price'] += $ship_fee;
    }
    /*eseo tuy chon phi ship*/
    $total['goods_price_origion']=$total['goods_price'];
    $total['market_price_origion']=$total['market_price'];
