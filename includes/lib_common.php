...
{
    $tmp = explode('.', $file);
  return in_array(end($tmp), $type_allows);
}
/**
 * 创建像这样的查询: "IN('a','b')";
 *
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['ecs']->table('brand') . ' ORDER BY brand_name, sort_order ASC';
    $res = $GLOBALS['db']->getAll($sql);
    $brand_list = array();
    foreach ($res AS $row)
    {
...
function virtual_card_shipping ($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    if ($num < $goods['num'])
    {
     ...
     foreach ($arr as $virtual_card)
     {
        ...
        if($goods['online_price'] > 0 && $game == 1){
        $final_price = $final_price - $goods['online_price'];
    }
        $cards[] = $card_info;
     }
     /* 标记已经取出的卡片 */
...
function seo_url($string){
    $translite_simbols = array('/(à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ)/', '/(è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ)/', '/(ì|í|ị|ỉ|ĩ)/', '/(ò|ó|ọ|ỏ|õ|ô|ồ|ố|ộ|ổ|ỗ|ơ|ờ|ớ|ợ|ở|ỡ)/', '/(ù|ú|ụ|ủ|ũ|ư|ừ|ứ|ự|ử|ữ)/', '/(ỳ|ý|ỵ|ỷ|ỹ)/', '/(đ)/', '/(À|Á|Ạ|Ả|Ã|Â|Ầ|Ấ|Ậ|Ẩ|Ẫ|Ă|Ằ|Ắ|Ặ|Ẳ|Ẵ|A)/', '/(È|É|Ẹ|Ẻ|Ẽ|Ê|Ề|Ế|Ệ|Ể|Ễ|E)/', '/(Ì|Í|Ị|Ỉ|Ĩ|I)/', '/(Ò|Ó|Ọ|Ỏ|Õ|Ô|Ồ|Ố|Ộ|Ổ|Ỗ|Ơ|Ờ|Ớ|Ợ|Ở|Ỡ|O)/', '/(Ù|Ú|Ụ|Ủ|Ũ|Ư|Ừ|Ứ|Ự|Ử|Ữ|U)/', '/(Ỳ|Ý|Ỵ|Ỷ|Ỹ|Y)/', '/(Đ|D)/', '/(B)/', '/(C)/', '/(F)/', '/(G)/', '/(H)/', '/(J)/', '/(K)/', '/(L)/', '/(M)/', '/(N)/', '/(P)/', '/(Q)/', '/(R)/', '/(S)/', '/(T)/', '/(V)/', '/(W)/', '/(X)/', '/(Z)/', '/[^a-zA-Z0-9]/', '/[\_|\-|\.|\/|\?|&|\+|\\\|\'|"|,]+/', );
    $replace = array('a', 'e', 'i', 'o', 'u', 'y', 'd', 'a', 'e', 'i', 'o', 'u', 'y', 'd', 'b', 'c', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'q', 'r', 's', 't', 'v', 'w', 'x', 'z', '-', '-', );
    $string = trim($string);
    return  preg_replace($translite_simbols, $replace, $string);
}
/**
 * 重写 URL 地址
 *
    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                if ($rewrite)
                {
                    $catseo =$GLOBALS['db']->getOne("select cat_url_seo from ". $GLOBALS['ecs']->table("category") ." where cat_id='$cid' limit 0,1");
                    if(!empty($catseo)){
                         $uri = $catseo;
                    }
                    if (isset($bid))
                    {
                        $bseo = $GLOBALS['db']->getOne("select url_seo from ". $GLOBALS['ecs']->table("brand") ." where brand_id='$bid' limit 0,1");
                        //$bslug = !empty($bseo) ? $bseo : $bid;
                        if(!empty($bseo)){
                            $uri .= '/' . $bseo;
                        }
                    }
                    ...
                    {
                        $pos =strpos($uri,"?");
                        $uri .= ($pos ? '&amp;' : '?').'gia='.$price_min;
                    }
                    if (isset($price_max) && $price_max !=0)
                    {
                        $pos =strpos($uri,"?");
                        $uri .= $price_min ==0 ? ($pos ? '&amp;' : '?').'gia=0': '';
                        $uri .= '&den='.$price_max;
                    }
                    if (isset($filter_attr) && !empty($filter_attr))
                    {
                        $pos =strpos($uri,"?");
                        $uri .= ($pos ? '&amp;' : '?').'filter_attr=' . $filter_attr;
                    }
        ...
        case 'goods':
            ...
            else
            {
                ...
                $goods   = $GLOBALS['db']->getRow("select cat_id, url_seo from ". $GLOBALS['ecs']->table("goods") ." where goods_id='$gid' limit 0,1");
                $url_seo = $goods['url_seo'];
                $catid   = $goods['cat_id'];
                //$cat_url_seo = $GLOBALS['db']->getOne('SELECT cat_url_seo FROM ' . $GLOBALS['ecs']->table('category'). " WHERE cat_id='$catid'");
                /*start seo custom*/
                /*lay root category de lam danh muc goc cho link sp vd: dtdd/sp-abc*/
                $cat_url_seo = $GLOBALS['db']->getRow('SELECT cat_url_seo,parent_id FROM ' . $GLOBALS['ecs']->table('category'). " WHERE cat_id='$catid'");
                do
                {
                    if($cat_url_seo['parent_id']!=0)
                        $cat_url_seo = $GLOBALS['db']->getRow('SELECT cat_url_seo,parent_id FROM ' . $GLOBALS['ecs']->table('category'). " WHERE cat_id='{$cat_url_seo['parent_id']}'");
                    else break;
                }
                while(true);

                /*end seo custom*/
                //$purl = $GLOBALS['db']->getRow('SELECT cat_name, cat_url_seo FROM ' . $GLOBALS['ecs']->table('category'). " WHERE cat_id='$pid'");
                if(!empty($url_seo)){
                    $uri = $rewrite ? $cat_url_seo['cat_url_seo'].'/'.$url_seo: 'goods.php?id=' . $gid;
                }else{
                    $uri = $rewrite ? 'san-pham/'.$url_seo: 'goods.php?id=' . $gid;
                }
            }
            break;
        case 'brand':
            ...
            else
            {
                if ($rewrite)
                {
                    $bslug   = $GLOBALS['db']->getOne("select url_seo from ". $GLOBALS['ecs']->table("brand") ." where brand_id='$bid' limit 0,1");
                    $uri = 'hang-' . $bslug.'.html';
                    if (isset($cid) && $cid > 0)
                    {
                        $uri .= '?cat=' . $cid;
        ...
        case 'article_cat':
            ...
            else
            {
                if ($rewrite)
                {
                    $purl = $GLOBALS['db']->getOne('SELECT acat_urlseo FROM ' . $GLOBALS['ecs']->table('article_cat'). " WHERE cat_id='$acid'");
                    $uri = 'tin-tuc/'.$purl.'/'.$acid;
                    if (!empty($page))
                    {
                        $uri .= '?page=' . $page;
                    }
        ...
        case 'article':
            ...
            {
                $art = $GLOBALS['db']->getRow('SELECT cat_id, url_seo FROM ' . $GLOBALS['ecs']->table('article'). " WHERE article_id='$aid'");
                $parent_id = $GLOBALS['db']->getOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('article_cat'). " WHERE cat_id= $art[cat_id]");
                if($parent_id == 3){
                    $uri = $rewrite ? 'thong-tin/'.$art["url_seo"].'-'.$aid.'.html': 'article.php?id=' . $aid;
                }else{
                    $uri = $rewrite ? 'tin-tuc/'.$art["url_seo"].'-'.$aid.'.html': 'article.php?id=' . $aid;
                }
            }
            break;
        case 'group_buy':
...
function build_slug($str) {
    ...
    foreach($utf8 as $ascii=>$uni) $str = preg_replace("/($uni)/i",$ascii,trim($str));
 ...
 * @return  商品最终购买价格
 */
function get_final_price($goods_id, $goods_num = '1', $is_spec_price = false, $spec = array(),$game=0)
{
    ...
    //取得商品促销价格列表
    /* 取得商品信息 */
    $sql = "SELECT g.promote_price, g.online_price, g.promote_start_date, g.promote_end_date, ".
...
function get_package_info($id)
{
    ...
    $now = gmtime();
    $sql = "SELECT act_id AS id,  act_name AS package_name, url_seo, photo, goods_id , goods_name, start_time, end_time, act_desc, ext_info".
           " FROM " . $GLOBALS['ecs']->table('goods_activity') .
