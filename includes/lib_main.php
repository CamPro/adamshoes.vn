...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    = '<li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="."><span property="name">' . $GLOBALS['_LANG']['home'] . '</span></a> <meta property="position" content="1"></li>';
    /* 根据文件名分别处理中间的部分 */
    if ($filename != 'index')
    {
        /* 处理有分类的 */
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' › <li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="' . build_uri($type, $args, $val['cat_name']) . '"><span property="name">' .
                                    htmlspecialchars($val['cat_name']) . '</span></a> <meta property="position" content="2"></li>';
                }
            }
        }
        /* 处理无分类的 */
        else
        {
            ...
            if ('group_buy' == $filename)
            {
                ...
                $args       = array('gbid' => '0');
                $ur_here   .= ' › <li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="nhom-mua"><span property="name">' .
                                $GLOBALS['_LANG']['group_buy_goods'] . '</span></a> <meta property="position" content="2"></li>';
            }
            /* 拍卖 */
            elseif ('auction' == $filename)
            {
                ...
                $args       = array('auid' => '0');
                $ur_here   .= ' › <li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="auction.php"><span property="name">' .
                                $GLOBALS['_LANG']['auction'] . '</span></a> <meta property="position" content="2"></li>';
            }
            /* 夺宝 */
            elseif ('snatch' == $filename)
            {
                ...
                $args       = array('id' => '0');
                $ur_here   .= ' › <li property="itemListElement" typeof="ListItem"><a href="snatch.php"><span property="name">' . $GLOBALS['_LANG']['snatch_list'] . '</span></a> <meta property="position" content="2"></li>';
            }
            /* 批发 */
            elseif ('wholesale' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= '› <li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="wholesale.php"><span property="name">' .
                                $GLOBALS['_LANG']['wholesale'] . '</span></a> <meta property="position" content="2"></li>';
            }
            /* 积分兑换 */
            elseif ('exchange' == $filename)
            {
                ...
                $args       = array('wsid' => '0');
                $ur_here   .= ' › <li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="exchange.php"><span property="name">' .
                                $GLOBALS['_LANG']['exchange'] . '</span></a> <meta property="position" content="2"></li>';
            }
            /* 其他的在这里补充 */
        }
    }
...
function getID_parent_cat($catid){
    $cat_id = $GLOBALS['db']->getOne("SELECT parent_id FROM " . $GLOBALS['ecs']->table('category')." WHERE cat_id = '$catid'");
    return $cat_id;
}
/**
 * 获得指定分类的所有上级分类
...
function build_urhere($arr, $type = 'category')
{
    ...
    foreach ($arr AS $val)
    {
        ...
        $str .= ' › <li property="itemListElement" typeof="ListItem"><a property="item" typeof="WebPage" href="' . build_uri($type, $args). '"><span property="name">' . htmlspecialchars($val['cat_name']) . '</span></a> <meta property="position" content="2"></li>';
    }
    return $str;
}
/**
                        ...
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    {
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'display' => $display_type);
            break;
        case 'article_cat':
            $uri_args = array('acid' => $cat, 'sort' => $sort, 'order' => $order, 'acatslug'=> $_REQUEST['turl']);
            break;
...
function assign_comment($id, $type, $page = 1)
{
    ...
    $sql = 'SELECT comment_id,email,user_name, content,comment_rank,add_time  FROM ' . $GLOBALS['ecs']->table('comment') .
            " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0".
    ...
    if ($ids)
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($res))
        {
            ...
            $arr[$row['parent_id']]['re_content'] = $row['content'];
        }
    }
    /* 分页样式 */
    //$pager['styleid'] = isset($GLOBALS['_CFG']['page_style'])? intval($GLOBALS['_CFG']['page_style']) : 0;
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('ym',            explode(',', $GLOBALS['_CFG']['ym']));    $smarty->assign('msn',           explode(',', $GLOBALS['_CFG']['msn']));
    $smarty->assign('skype',         explode(',', $GLOBALS['_CFG']['skype']));
    //mode dev
    $stats_code = (isset($_COOKIE['_tester']) && $_COOKIE['_tester'] == 1) ? '' : $GLOBALS['_CFG']['stats_code'];
 ...
 * @return  array
 */
function article_categories_tree($parent_id = 0)
{
    /*
     判断当前分类中全是是否是底级分类，
     如果是取出底级分类上级分类，
