        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                ...
                if(!empty($row["ad_link"])){
                    $ads[] = "<a target='_blank' href='ads.php?ad_id=$row[ad_id]&amp;uri=" .urlencode($row["ad_link"]). "'
                ><img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]' border='0' /></a>";
                }else{
                    $ads[] = "<img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]' border='0' />";
                }
                break;
            case 2: // CODE
                $ads[] = $row['ad_code'];
                break;
            case 3: // TEXT
                $ads[] = "<a href='ads.php?ad_id=$row[ad_id]&amp;uri=" .urlencode($row["ad_link"]). "'
...
function insert_cart_number()
{
    ...
    $GLOBALS['smarty']->assign('number_cart', $number );
    $val = $GLOBALS['smarty']->fetch('library/cart_number.lbi');
    $GLOBALS['smarty']->caching = $need_cache;
    $GLOBALS['smarty']->force_compile = $need_compile;
    return $val;
}
