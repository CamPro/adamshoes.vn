...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.cat_id, g.goods_name, g.goods_name_style, g.market_price, g.goods_desc_short, g.is_hot, g.is_best, g.is_new, g.is_promote, g.is_special, g.is_os, g.goods_number, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.img_special, g.seller_note, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        foreach ($result AS $idx => $row)
        {
            if ($row['promote_price'] > 0)
            {
                ...
                if($promote_price > 0){
                $goods[$idx]['rate_sale'] =  100-round(($promote_price/$row['shop_price'])*100, 0);
                }
            ...
            $goods[$idx]['raw_price']    = $row['shop_price'];
            ...
            $goods[$idx]['is_os']        = $row['is_os'];
            $goods[$idx]['is_promote']   = $row['is_promote'];
            $goods[$idx]['text_status']  = $row['seller_note'];
            $goods[$idx]['goods_thumb2col']  = $row['img_special'];
            $goods[$idx]['stock']        = $row['goods_number'];
            $goods[$idx]['short_desc']   = nl2br($row['goods_desc_short']);
            ...
            $goods[$idx]['discount']     = price_format($discount);
...
/**
 * Lay ra 5 sp mobile moi ra, is_new = 1, o danh muc tin
 *
 * @access  public
 * @return  array
 */
function get_new_goods_mobile()
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "g.promote_start_date, g.promote_end_date,  g.goods_thumb " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            ' WHERE g.cat_id IN (110,111,112,113,114,115) AND g.is_new = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
    $sql .= ' ORDER BY g.add_time DESC LIMIT 5';
    $result = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['id']           = $row['goods_id'];
        $goods[$idx]['name']         = $row['goods_name'];
        $goods[$idx]['org_price']   = price_format($row['org_price']);
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
    }
...
/**
 * Lay ra 8 sp phu kien HOT = 1 ra home
 *
 * @access  public
 * @return  array
 */
function get_acchome()
{
    $cats = get_children(5);
    $where = !empty($cats) ? "AND ($cats OR " . get_extension_goods(5) . ") " : '';

     $data = read_static_cache('acchome');
    if ($data === false)
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                ' WHERE g.is_hot = 1 '.$where.' AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
        $sql .= ' ORDER BY g.goods_id DESC LIMIT 8';
        $result = $GLOBALS['db']->getAll($sql);
        ...
        write_static_cache('acchome', $goods);
    }
    else
    {
        $goods = $data;
 ...
 * @return  array
 */
function get_goods_by_catid($cat_id)
{
    $cats = get_children($cat_id);
    $where = !empty($cats) ? "AND $cats " : '';

     $data = read_static_cache('goodsbycatid'.$cat_id);
    if ($data === false)
    {

        $sql = 'SELECT g.goods_id, g.goods_name,g.goods_desc_short, g.market_price, g.seller_note, g.shop_price AS org_price, g.promote_price, ' .
                    "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                    "g.promote_start_date, g.promote_end_date,  g.goods_thumb " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                " WHERE g.is_on_sale = 1 $where AND g.is_delete = 0 " .
        $sql .= ' ORDER BY g.goods_id DESC LIMIT 10';
        $result = $GLOBALS['db']->getAll($sql);
        ...
        write_static_cache('goodsbycatid'.$cat_id, $goods);
    }
    else
    {
        $goods = $data;
...
/**
 * Lay danh san sp de xuat o category
 *
 * @access  public
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_number, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.seller_note, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, tg.laisuat, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
    ...
    if ($row !== false)
    {
        ...
        if ($row['promote_price'] > 0)
        {
            ...
            if ($row['online_price'] > 0){
                $online_final = $promote_price - $row['online_price'];
                $row['online_final'] = price_format($online_final);
            }
            $row['price_final'] = $promote_price;
        }
        else
        {
            $promote_price = 0;
            if ($row['online_price'] > 0){
                $online_final = $row['shop_price'] - $row['online_price'];
                $row['online_final'] = price_format($online_final);
            }
            $row['price_final']  = $goods['shop_price'];
        }

        $row['promote_price_org'] =  $promote_price;
        $row['promote_price'] =  price_format($promote_price);

        // Google Rating Offer Price format VND
        $row['offer_price'] = number_format($promote_price, 0, ',', '.');
        $row['sell_price'] = number_format($row['shop_price'], 0, ',', '.');
        ...
        if ($row['online_price'] > 0){
            $row['online_raw'] = $row['online_price'];
            $row['online_price'] = price_format($row['online_price']);
        }

        if ($row['tragop_price'] > 0){
            $row['tragop_price_raw'] = $row['tragop_price'];
            $row['tragop_price'] = price_format($row['tragop_price']);
        }
        ...
        $row['goods_desc_short']   = nl2br($row['goods_desc_short']);
        $row['goods_brief']   = nl2br($row['goods_brief']);

        $row['laisuat']      = $row['laisuat'] = NULL ? '' : $row['laisuat'] ;

        if(!empty($row['goods_desc_long'])){
            $cauhinh = explode("\n", str_replace("\r", '', $row['goods_desc_long']));
            $thongso = array();
            foreach ($cauhinh as $val) {
                $line = explode(":", $val);
                $thongso[$line[0]] = $line[1];
            }
            $row['goods_cauhinh'] = $thongso;
        }else{
            $row['goods_cauhinh'] = '';
        }
        $row['showtragop'] = $row['is_tragop'];
        $row['is_os'] = $row['is_os'];
        $row['preoder'] = $row['is_preoder'];
        $row['timegift'] = $row['timegift'];
        $row['on_sale'] = $row['is_on_sale'];
        $row['thutuc'] = $row['thutuctg'];
...
function get_tragop($nhataichinh, $goods_id){
    $sql = "SELECT nhataichinh, laisuat, tratruoc, kyhanvay, thutuc, thutuc_info, is_money, endday FROM " .$GLOBALS['ecs']->table('tragop'). " WHERE goods_id = '$goods_id' AND nhataichinh = '$nhataichinh'";
     $tragop = $GLOBALS['db']->getRow($sql);
    return $tragop;
}

function get_thongsochinh($goods_id)
{
     $sql = "SELECT  a.attr_name, a.attr_type, g.attr_value " .
            'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('attribute') . ' AS a ON a.attr_id = g.attr_id ' .
            "WHERE g.goods_id = '$goods_id' AND a.is_main = 1 " .'ORDER BY a.sort_order';
    $res = $GLOBALS['db']->getAll($sql);
    // $arr = array();
    // foreach ($res AS $row)
    // {
    //     $arr['attr_name'] = $row['attr_name'];
    //     $arr['attr_value'] = $row['attr_value'];
    // }
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.is_preoder, g.goods_desc_short, g.goods_name, g.market_price, g.is_hot, g.is_best, g.is_new, g.is_promote, g.is_os,  g.is_special, g.goods_number, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.seller_note, g.online_price, g.goods_thumb, g.goods_img ' .
            " FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            " LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            ' WHERE g.is_best = 1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['brief']        = $row['goods_brief'];
        $goods[$idx]['short_desc']   = nl2br($row['goods_desc_short']);
        $goods[$idx]['is_preoder']   = $row['is_preoder'];
        $goods[$idx]['text_status']  = $row['seller_note'];
        $goods[$idx]['is_best']      = $row['is_best'];
        $goods[$idx]['is_hot']       = $row['is_hot'];
        $goods[$idx]['is_new']       = $row['is_new'];
        $goods[$idx]['is_promote']   = $goods[$idx]['promote_price'] != '' ? $row['is_promote'] : 0;
        $goods[$idx]['is_special']   = $row['is_special'];
        $goods[$idx]['is_os']        = $row['is_os'];
        $goods[$idx]['stock']        = $row['goods_number'];
        $goods[$idx]['laisuat']      = $row['is_os'] == 3 ? 3 : $row['is_os'];
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['short_name']   = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                        sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
...
function get_goods_fittings($goods_list = array())
{
    ...
    $sql = 'SELECT gg.parent_id, ggg.goods_name AS parent_name, gg.goods_id, gg.goods_price, g.goods_name, g.goods_thumb, g.goods_img, g.is_promote, g.promote_price, g.promote_start_date, g.promote_end_date, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
           $promote_price = bargain_price($row['promote_price'], $row['promote_start_date'], $row['promote_end_date']);
           $arr[$temp_index]['promote_price'] = $promote_price > 0 ? price_format($promote_price) : '';
           $raw_price =  $promote_price > 0 ? $promote_price : 0; // gia chua duoc formart
        }
        else
        {
            $arr[$temp_index]['promote_price'] = '';
            $raw_price = $row['shop_price'];
        }

        $arr[$temp_index]['raw_price']         = $raw_price; //gia chua formart
