...
class cls_template
{
    ...
    // tabs, line breaks, and white space
    //$output = preg_replace( array('/\>[^\S ]+/s','/[^\S ]+\</s','/(\s)+/s'), array('>','<','\\1'), $output );
    // no tabs, no new lines, no comments (<!--.*?-->).
   ...
   // $output = preg_replace('/^\\s+|\\s+$/m', '', $output);
    ...
    //$output = preg_replace('/>(\\s(?:\\s*))?([^<]+)(\\s(?:\s*))?</','>$1$2$3<', $output);
   // $output = preg_replace('#/\*.*?\*/#', '', $output); #comments css
   // $output = preg_replace('/[\s]*([\{\},;:])[\s]*/', '\1', $output); #spaces on css
   ...
   function compressHTML($html) {
    ...
    function select($tag)
    {
        ...
        elseif (substr($tag,0,4) == 'siy:') { // david@Shopiy
