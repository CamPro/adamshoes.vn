...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    else
    {
        ...
        // 更新：添加到购物车
        if (addto_cart($goods->goods_id, $goods->number, $goods->spec, $goods->parent,$goods->game))
        {
    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if ($user->login($_POST['username'], $_POST['password'],isset($_POST['remember'])))
            {
                ...
                if ($db->getOne($sql) > 0)
                {
                    ecs_header("Location: thanh-toan\n");
                }
                else
                {
            ...
            {
                $_SESSION['login_fail']++;
                show_message($_LANG['signin_failed'], '', 'dang-nhap-thanh-vien');
            }
        }
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
                ecs_header("Location: thong-tin-giao-hang\n");
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
        ...
        ecs_header("Location: dang-nhap-thanh-vien\n");
        exit;
    }
...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
    ...
    unset($_SESSION['flow_consigneef']); // 清除session中保存的收货人信息
...
// STEP CART edit by Nobj, 1 buoc
elseif ($_REQUEST['step'] == 'finished'){

    $cadd     = isset($_POST['address']) ? $_POST['address'] : '';
    $shop     = isset($_POST['address2']) ? $_POST['address2'] : '';
    $shipmethod = isset($_POST['shipmethod']) ? intval($_POST['shipmethod']) : 1;

    $customer =  array(
        'name'          => isset($_POST['fullname']) ? filter_var(trim($_POST['fullname']), FILTER_SANITIZE_STRING) : '',
        'sex'           => isset($_POST['sex']) ? intval($_POST['sex']) : 0,
        'mobile'        => isset($_POST['mobile']) ? filter_var(trim($_POST['mobile']), FILTER_SANITIZE_STRING) : '',
        'email'         => isset($_POST['email']) ? trim($_POST['email']) : '',
        'shipmethod'    => $shipmethod,
        'paymethod'     => isset($_POST['paymethod']) ? intval($_POST['paymethod']) : 0,
        'province_id'   => isset($_POST['province']) ? intval($_POST['province']) : 0,
        'city_id'       => isset($_POST['city']) ? intval($_POST['city']) : 0,
        'address'       => $shipmethod == 1 ? $shop : $cadd,
        'order_note'    => isset($_POST['order_note']) ? filter_var(trim($_POST['order_note']), FILTER_SANITIZE_STRING) : '',
    );
    //validate thong tin khach hang lan 2
    $errorMsg = '';

    if($customer['name'] == '' || $customer['mobile'] == ''){
        $errorMsg .= 'Chưa điền tên hoặc số điện thoại';
    }
    if($customer['email'] != '' && is_email($customer['email']) == false){
        $errorMsg .= 'Email không hợp lệ';
    }
    if($customer['address'] == ''){
        $errorMsg .= 'Bạn chưa chọn hoặc nhập địa chỉ nhận hàng<br/>';
    }

    $lmobile = strlen($customer['mobile']);
    $pmobile = substr($customer['mobile'],  0, 1);
    if($lmobile > 0 && ($lmobile  < 10 || $lmobile  > 11 || $pmobile != 0)){
        $errorMsg .= 'Số điện thoại không hợp lệ <br/>';
    }

    if (!empty($errorMsg))
    {
        show_message($errorMsg, $_LANG['back_to_cart'], 'flow.php');
        exit;
    }
    ...
    $sql = "INSERT INTO " . $ecs->table('customer') . "(customer_name, customer_mobile, customer_email, customer_address, customer_sex, customer_province)" .
                                              " VALUES ('$customer[name]', '$customer[mobile]', '$customer[email]', '$customer[address]', '$customer[sex]', '$customer[province_id]')";
    $db->query($sql);
    ...
    $order_type = 1;
    $order_device = 0; // 0|desktop 1|mobile
    // $order_type: 0|PreOder, 1|Order, 2|ShockOrder
    // Insert Order Table
         $sql = "INSERT INTO " . $ecs->table('order') . "(order_code, order_type, customer_id,  add_time, order_device, order_note, shipmethod, paymethod,order_subtotal,order_ship_fee,order_total)" .
                                               " VALUES ('$order_code', '$order_type', '$customer_id', '".gmtime()."', '$order_device', '$customer[order_note]', '$customer[shipmethod]', '$customer[paymethod]' ,'".$cart_goods['total']['subtotal_origion']."','".$cart_goods['total']['shipping_fee_origion']."','".$cart_goods['total']['goods_price_origion']."'   )";
    ...
    foreach ($cart_goods['goods_list'] as $goods) {
        $sql = "INSERT INTO " . $ecs->table('order_product') . "(order_id, giasoc_id, goods_id, product_sn, product_url, product_name, product_price, product_number, product_gift, product_attr)" .
                                               " VALUES ('$order_id', '', '$goods[goods_id]', '$goods[goods_sn]', '', '$goods[goods_name]', '$goods[org_price]', '$goods[goods_number]', '$goods[goods_gift]', '$goods[goodsattr]')";
        $res = $db->query($sql);
    }
    ...
    /*if(!empty($res)){
        ...
        if($allow_sendsms){
            ...
            $content = 'Co don hang tu '.$csex.$cname.', '.$customer['mobile'];
            $content = utf8convert($content); //chuyen qua ko dau
            $sms = new SpeedSMSAPI();
            $sms->sendSMS($phones, $content);
        }
    }*/
    //Chuyen toi trang flow done, xuat ra lai thong tin khach hang da dien. Thong bao thanh cong!
    ...
    //xu li thanh toan va shop de show
    $customer['sex'] = $customer['sex'] == 1 ? 'Anh' : 'Chị';
    $customer['shipmethod'] = $customer['shipmethod'] == 1 ? 'Nhận hàng tại cửa hàng' : 'Nhận hàng tại địa chỉ';
    if($customer['paymethod'] == 1)
        $customer['paymethod']  = 'Thanh toán trực tiếp tại cửa hàng';
    elseif($customer['paymethod'] == 2)
        $customer['paymethod']  = 'Thẻ ATM có Internet Banking';
    else
        $customer['paymethod']  = 'Thẻ VISA, Master';

    $customer['province']   = $GLOBALS['db']->getOne("SELECT region_name FROM ". $GLOBALS['ecs']->table('region') ." WHERE region_id = $customer[province_id] AND region_type = 1 LIMIT 1");
    $customer['city']       = $GLOBALS['db']->getOne("SELECT region_name FROM ". $GLOBALS['ecs']->table('region') ." WHERE region_id = $customer[city_id] AND region_type = 2 LIMIT 1");
