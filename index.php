...
$cache_id = sprintf('%X', crc32($_SESSION['user_rank'] . '-' . $_CFG['lang'].'-'.$_COOKIE['client'].'-'.$_GET['home2']));
...
if (!$smarty->is_cached($display_mode, $cache_id))
{
    ...
    $smarty->assign('acchome',   get_acchome());

    // banner home for mobile
    if(isset($_COOKIE['client']) || isset($_GET['client'])){
        $sql = 'SELECT ad_name, content, url FROM ' . $ecs->table("ad_custom") . ' WHERE ad_status = 1 AND ad_type = 0 ORDER BY sort ASC';
        $ad = $db->getAll($sql);
        $smarty->assign('ads_mobile', $ad);
